...
describe("tolerant surfaces stay tolerant", () => {
  ...
  it("clearDaemonDirty and markReviewed are best-effort writes with fallbacks", async () => {
    ...
    expect(tolerant).toHaveBeenCalledWith("mark_reviewed", { agent_id: "agent-1" }, true);
    expect(strict).not.toHaveBeenCalled();
  });
});
